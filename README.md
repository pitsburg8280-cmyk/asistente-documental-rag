...
Sistema de **Generación Aumentada por Recuperación (RAG)** completamente funcional y local. Permite interactuar con documentos PDF privados utilizando Modelos de Lenguaje de Gran Escala (LLMs) de código abierto, garantizando respuestas precisas y minimizando alucinaciones.
...
graph LR
    subgraph "Ingesta y Procesamiento"
        A[📄 PDFs Locales] --> B[✂️ Chunking Avanzado]
        B --> C[🧠 Embeddings all-MiniLM-L6-v2]
    end

    C --> D[💾 ChromaDB Vectorial]
